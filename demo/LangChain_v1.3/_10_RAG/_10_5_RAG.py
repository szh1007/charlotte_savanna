...
rprint(
    f"Milvus_charlotte_docs_stat: {client.get_collection_stats(MILVUS_COLLECTION_NAME)}"
)
...
def generate_answer(query: str):
    ...
    for i, result in enumerate(query_results):
        ...
        context_blocks.append(
            f"[参考{i} | chunk_id={chunk_id} | source={source}]\n{text}"
        )
...
while True:
    query = input(
        "\n\n"
        + "-" * 25
        + "欢迎使用 Charlotte 智能问题助手"
        + "-" * 25
        + "\nQuestion: "
    )
    # "你好, 我叫Charlotte,
    # 帮我查一下项目 charlotte_savanna 主要用到了什么技术栈"
