...
class GetWeatherFields(BaseModel):
    location: str = Field(
        description="[High Priority] The location to get the weather for."
    )
    time: str = Field(
        description="[High Priority] The time to get the weather for.", default="today"
    )
...
def get_weather(location: str, time: str = "today") -> str:
    ...
    (convert docstring to tool description)

    Args:
        location: The location to get the weather for.
        time: The time to get the weather for. Defaults to "today".

    Returns:
        The weather description for the given location and time.
    """
    return f"The weather in {location} on {time} is rainy."


# 绑定工具
# tool_choice = "none" 绝不调用 / "auto" 自动 / "required" 必须调用
# (可强制调用指定的工具, 设置为工具名即可)
...
# 1.工具描述要尽可能清晰
# 2.每个工具职责划分要边界清晰
# 3.处理工具调用失败 - Agent级使用Prompt安排重试 /
#   调用级重试@retry / 最好让工具返回字符串 / 异步调用
