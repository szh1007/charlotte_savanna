...
def load_prompt(file_path):
    ...
    with open(file_path, encoding="utf-8") as f:
        ...
        yaml.load(): 不安全, 会解析 YAML 中的自定义对象/执行代码,
          如果加载的 YAML 文件被恶意篡改(比如插入了执行系统命令的代码),
          会导致服务器被攻击, 数据泄露;
        yaml.safe_load(): 仅解析 YAML 标准数据类型(字符串, 数字, 字典, 列表, 布尔值等),
          完全禁止解析/执行任何自定义对象, 函数, 代码, 从根源避免安全风险.
        """
...
# Path(__file__) 当前文件的Path prompts.py
#   .parents -> [agent,deep_search_0415] -> 所有的父目录
#   .parent . parent  = index + 1
root_path = Path(__file__).parents[1]  # 项目根目录

# 拼接提示词配置文件的完整路径(根目录/prompt/prompts.yml)
prompt_file_path = root_path / "prompt" / "prompt.yaml"
...
print(
    f"main_agent_config: {main_agent_config} , \nsub_agents_config: {sub_agents_config}"
)
