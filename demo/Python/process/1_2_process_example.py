...
def speak(n, lock):
    for i in range(n):
        ...
        print(f"{current_process().name}-{i + 1}: {os.getpid()}-{os.getppid()}")
    ...
    while True:
        try:
            with open("log.txt", "r", encoding="utf-8") as log:
                sum_lines = len(log.readlines())
        except FileNotFoundError:
            sum_lines = 0

        print(f"{current_process().name}-{os.getpid()}-{os.getppid()}: log.txt 共{sum_lines}行")
        time.sleep(1)
