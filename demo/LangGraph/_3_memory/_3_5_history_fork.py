...
def poem_node(state: StructuredOutputState) -> OverAllState:
    ...
    poem = model.invoke(
        [HumanMessage(f"请写一首关于{state['topic']}的七言诗, 不展示分析过程直接输出诗")]
    ).content
...
def joke_node(state: StructuredOutputState) -> OverAllState:
    ...
    joke = model.invoke(
        [HumanMessage(f"请写一个关于{state['topic']}的笑话, 同时还是解释笑点, 并解释笑点背景")]
    ).content
    return {"output": f"主题: {state['topic']}\n\n笑话: {joke}"}
...
with PostgresSaver.from_conn_string(PG_DB_URL) as checkpointer:
    ...
    # rprint(
    #     graph.invoke(
    #         {"username": "charlotte", "user_input": "写一首关于猫的诗"}, config=config
    #     )
    # )
    ...
    # 1.如果指定为节点自身, 则表示修改自身节点的输入, 【重新生成】后续的输出
