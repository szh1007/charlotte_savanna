...
> 原设计（只读直连 minimall MySQL + demo 自建业务表）已被 ADR-0008 推翻，改为「业务 internal API + 内部网关 + 身份服务」三层对接，业务代码独立为 `CharService/` 子项目。
> 本 issue 的内容拆解为 `.scratch/CharService/issues/01~10`（10 个垂直切片），规格见 `.scratch/CharService/PRD.md`。
> 下方原文保留作为设计演进记录。
