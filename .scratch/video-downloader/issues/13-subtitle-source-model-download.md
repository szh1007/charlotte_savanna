...
**验收标准：**
- [ ] API 契约: `POST /api/summarize` 新增 `subtitle_source`（`official` / `model`,
...
- [ ] 测试: 沿用 HTTP seam 模式 + 引擎 mock（模型下载引擎为独立调用点替换, 驱动
      ...
      pytest 全绿
- [ ] 文档同步: DESIGN.md 内容获取管线章节更新; CONTEXT.md / ADR-0006 / PRD
      Phase 3 状态更新（术语与决策已就绪）

## Comments

- 2026-08-21: issue 13 创建（to-tickets, 单 issue 交付）。需求与用户故事见
  `.scratch/subtitle-source-model-download/PRD.md`（24 条用户故事, Testing
  Decisions 含接缝清单）, 决策见 `project/video_downloader/docs/adr/
  0006-subtitle-source-and-model-download.md`。未实施。
