...
- [ ] 框架现有测试全绿（703 个，一个不能少）
- [ ] 新增测试覆盖：工具提供者被正确调用、载荷原样透传、提示词目录参数生效
- [ ] 通用性测试通过：同装配装两个不同业务都能跑通
- [ ] `agent/loop.py` **零改动**（这是本 issue 的核心验收点 —— 如果加接入点必须改主循环，说明接入点设计错了，要停下来重新想）
- [ ] 命令行现有行为不变（跑一次 `python -m CharAgent.client -q "现在几点?"` 正常）
