"""业务侧的护栏插件: 挂在「工具执行前」那一个点上的四条规则 (L2 的验收核心).
...
四条规则 (前两条的数为什么是那两个数, 见下面):

- **写操作预算 8 次**: 一次正常的购物流程 (加购 → 改量 → 下单 → 取消 → 申请退款)
  大约 6 到 8 次写操作, 8 不误伤; 而模型跑飞时连环写操作**一定会撞上它**.
- **单笔金额上限 5000 元**: 卡在「日常购买」与「大额消费」之间. 商城在售的五件
  ...
  这不是误伤而是本意 —— 而且到 L3 它**没有**变成"挂起 → 买家点确认" (issue 37
  定下的): 超限仍然当场拒. 同一条金额的规矩下, "本人拍板"有两种做法, 这里挑的是
  让他**自己动手到结算页**完成, 而不是他点一下由助手替他做 —— 那是另一件事
  (下单挂起) 管的分寸, 两者别混.
- **代付挂起** (issue 35): 调 `pay_my_order` 的那一步**不执行**, 整次运行停在这里
  等买家本人点头 —— 这是 L2 那句「框架级的人工确认是 L3 的事」兑现的地方, 也是
  后两条与前两条的根本不同: 前两条是**拒绝** (当场有结论, 工具不跑), 后两条是
  **挂起** (还没轮到它, 结论由本人给, 给完从存档点接着跑).
- **下单挂起** (issue 37): 金额没超时, 调 `place_order` 的那一步同样**不执行**,
  等买家在页面上点一下确认. 这条与代付是**同一套机制的两个形态** —— 代付要本人
  补一份数据 (支付密码), 下单只要一个是 / 否; 前端那张卡也据此长成两个样子
  (一个带密码框, 一个只有两个按钮), 而框架那一侧一个字都没多改. 两形态各对应
  一条业务规则, 这正是 L3b 要证明的"框架级确认能覆盖多个场景".
...
# 下单挂起时给买家看的那句话 (issue 37). 这张卡**不要输入框** (`needs=()`),
# 所以这句话只需要让他知道"点什么": 卡上就两个按钮, 选一个.
ORDER_APPROVAL_PROMPT = "确认要下这一单吗?"
...
class WriteGuardrail:
    """四条规则合一个插件: 两条当场拒绝 (预算 / 金额) + 两条挂起等人 (代付 / 下单).
    ...
    async def __call__(self, *, tool: Tool, **kwargs: Any) -> Decision | None:
        ...
        该当场说「不行」, 不该弹一张确认卡让买家白输一次密码 (下单那条的金额上限
        同理, 见 `_order_decision`). 跨插件的同一个方向由框架保证 —— `decide()`
        不看注册顺序, 见 `CharAgent/hooks/registry.py`.
        ...
        if tool.name == PLACE_ORDER_TOOL:
            # 下单: 先判金额 (超了当场拒), 没超就**挂起等买家点一下确认**
            # (issue 37). 它占一个写名额 (与 L2 同一条口径: 放行到"能做成"就记
            # 一笔), 见 `_order_decision`.
            return await self._order_decision()
        # 其余写操作当场放行: 记账与返回在同一个事件循环步里完成 (中间没有 await),
        # 所以同一轮的几次并发调用不会互相盖掉账本.
        self._used += 1
        return None

    async def _order_decision(self) -> Decision:
        """下单这一步的两条路: 金额超限 → 当场拒; 没超 → 挂起等买家点确认并记一笔.

        为什么判金额排在挂起**之前**: 挂起之后买家在卡上点一下确认, 这一步就真的
        执行了, 而金额上限是**护栏自己的**规矩 (商城那边没有这道闸) —— 先弹卡等于
        他点一下就把 5000 元的上限绕过去了. 所以超限的那一单连卡都不弹.

        为什么这一笔**要**记账 (与 L2 时同一条口径 —— 那时这一步是当场放行的, 挂起
        换掉的是"什么时候执行", 不是"要不要执行"): 下单最终真的会发生, 而护栏不知道
        自己是在挂起还是在补做 —— 框架只拿 `approved` 抵消「需确认」那一种裁决
        (`loop._execute_one`), 恢复段因此会**再进这个方法一次** (金额也再判一次).
        记在这里, 把那一次真的执行算进去, 才是"一次写操作记一笔". 说清代价: 买家
        在卡上点**取消**时这一笔也已经记了 —— 那一段就少一个名额. 方向是保守的
        (少放行一次而不是多放行一次), 而且它落在同一次运行的尾巴上.

        代付那条不记 (issue 35 定的): 它换掉的是"当场拒绝", 一次没做成的付款不该
        占额度. 两条挂起的记账不同不是笔误, 是它们各自替换掉的东西不同.

        Note:
            恢复那一段会**再问一次**金额: 挂起期间买家又往车里加了一件贵的, 这一单
            在恢复时会被同一句话拒掉 —— "批准"抵消的只是挂起, 不该把金额那关一并
            放行.
        """
        ...
        self._used += 1
        return Decision.requires_approval(prompt=ORDER_APPROVAL_PROMPT, needs=())
