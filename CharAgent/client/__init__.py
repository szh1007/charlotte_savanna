"""client 包: 命令行演示入口 `python -m CharAgent.client` (issue 10, P0 整体验收线).

一句话理解: 这是「不依赖 server 也能把框架跑起来」的那扇门 —— 终端里问一句,
agent 带着工具去查去算, 过程实时打在屏幕上, 中途 Ctrl-C 能打断, 再 `/resume`
(或直接说一句「继续」) 就从断点接着跑. P0 的八个包在这里第一次被装配成一台真的
机器 (此前只有测试在组装它们).

设计依据 (CharAgent/docs):
- design/05-roadmap.md 的 P0 验收: CLI 跑通带工具问答 + checkpoint 中断续跑 +
  `pytest tests/` 全绿
- difficulties #3 (kill switch = `asyncio.Task.cancel` 即时打断) / #4 (事件流实时
  展示) / #5 (断点续跑不重复已完成动作) / ADR-0002 (存储配置切换)
