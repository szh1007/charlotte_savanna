...
- TRUNCATION_*_TEXT: length 截断后回填模型的指令 (role=system; 语义上
  ...
  均允许中间 system 消息)
- APPROVAL_ALREADY_PENDING_TEXT: 同一批里第二条「需人工确认」的调用被跳过时,
  回填给模型的失败原因 (#25 HITL: 一次只确认一条, 确认卡一次只弹一张)
...
# 同一批里「第二条要人工确认的调用」被跳过时回填给模型的失败原因 (#25 HITL).
#
# 为什么不是「一起挂起」: `Suspension.pending` 的形状允许一批, 但「一次确认配一份
# 一次性载荷」要求载荷归属无歧义 (ADR-0014). 于是挂起第一条, 其余按**普通工具失败**
# 回填 —— 模型收到后能理解「先处理那一条」, 而不是被同一张卡问两遍.
APPROVAL_ALREADY_PENDING_TEXT = (
    "这一步被跳过: 同一批里还有一条调用正等用户确认, 而一次只能确认一条 —— "
    "请先让用户处理那一条, 之后再看这一步还要不要做"
)
