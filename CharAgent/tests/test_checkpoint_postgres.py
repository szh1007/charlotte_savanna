...
def read_raw_row(pg_dsn: str, checkpoint_id: str) -> tuple | None:
...
def delete_frame(pg_dsn: str, checkpoint_id: str) -> None:
...
async def test_deleting_a_frame_orphans_its_branches(pg_dsn, pg_saver, pg_thread_id):
    ...
    这是 utils/ddl.py 里那个删除动作选择的效果 (ON DELETE SET NULL, 不是 CASCADE):
