...
框架里只有两个模块打日志 (别处都上抛给调用方): 本模块与 `app` 的取消端点.
共同点是「没有调用方可以上抛」—— server 层是进程里的最后一站, 一次运行的失败
只有两个去处 (客户端的事件流, 对方可能已经走了; 与进程日志), 而越界的取消请求
没有任何响应能把它说给自己人听 (回了 404 就说不出「是越界」). 所以前者对「没人
再保管的失败」记一笔 traceback, 后者对「取消别人的运行」记一笔警告; 除此之外
一律照旧上抛.
"""
...
class RunRegistry:
    ...
    字符串). 本模块只管「在册」这件事与它的收尾; 取消的 HTTP 语义 (已结束的
    run / 不存在的 run 各回什么) 在 `app.cancel_run` 与 RunNotFoundError 里.

    出册的唯一入口是任务自己的收尾回调 (finish), 所以「在册」与「在跑」永远
    同真同假 —— 不会出现一个已经没有任务在跑、却还挂在表里的幽灵条目.

    这句话有一个**一拍的例外**: 收尾回调是 asyncio 用 `call_soon` 排的 (任务结束
    之后的下一个循环才跑), 而这段时间里查表仍查得到那个句柄. 所以「在跑吗」这件事
    不能只问表 —— 还要看 `RunHandle.task` 的 `done()` (取消端点就是这么判的, 见
    `app.cancel_run`). 表管「这次运行还算不算数」, 任务管「它还在不在跑」.
    """
    ...
    def get(self, run_id: str) -> RunHandle | None:
        """查一次运行 (在跑则给句柄, 否则 None) —— 取消端点、排查与测试用."""
