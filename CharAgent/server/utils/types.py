...
**框架不解释 RunContext.payload**: 里面放什么 (权限 / 语言 / 页面来源...) 是业务
的事, 与 ToolProvider 那条纪律完全一致. 框架认识的是另外三个**结构**字段 ——
`thread_id` (会话分区), `tenant_id` / `user_id` (列出某个属主的会话时按它们
过滤), 而这三个都只是字符串: 框架拿它们做键与条件, 不解释它们在业务里指什么
(见 `agent/provider.py` 里 RunContext 的说明).
...
class ContextProvider(Protocol):
    ...
    async def provide(self, request: Request) -> RunContext:
        ...
        1. **认证**: 读令牌之类的凭据, 对不上就抛 `ServerAuthError` ——
           ...
           不存在」漏给调用方; 细节自己记日志.
        2. **取身份**: 当前属于哪个租户、哪个属主 (`tenant_id` / `user_id` 两个
           字段 —— 「列出我的会话」靠的就是它们); 其余业务私货放 `payload`.
        ...
        Returns:
            RunContext: 这次运行的上下文 (会话编号 + 属主身份 + 业务自己的载荷).
