...
**Sandbox**:
工具沙箱/最小权限：agent 只能访问白名单内的目标，业务数据的读写权限由业务系统侧收口。
_Avoid_: 隔离环境
...
**Order**:
业务系统（minimall）的订单数据，客服工具**经内部网关**以只读方式访问，不直连业务库。
_Avoid_: 订单记录

**Refund**:
退款，写操作中最高的风险级别：金额达阈值即挂起待审批，且**异步受理**（提交即返回退款单号，后台推进到账）。
_Avoid_: 退钱, 赔付
...
**Approval**:
内部审批记录：挂起的高危操作由**主管 / 风控**批准或拒绝——发起方是客服坐席，与审批方分离，同一人不得兼顾两侧。
_Avoid_: 审批单, 用户确认

**Gateway**:
内部 API 网关，agent 访问业务系统的唯一入口，在此完成认证、限流、审计与转发。
_Avoid_: 代理, 中间件

**Principal**:
委托身份，即「某个用户 + 代其操作的客服 agent」这一组合，决定工具能读写谁的数据。
_Avoid_: 身份, 调用方

**Confirmation**:
用户本人对终态操作的确认，以一次性凭据表达；与内部审批是两条不同的挂起路径。
_Avoid_: 二次确认, 用户审批
