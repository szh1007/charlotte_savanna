...
python -m CharAgent.client --backend postgres    # 换快照后端（redis / postgres 需本机服务）
python -m CharAgent.client --history             # 看本会话的快照历史表
python -m CharAgent.client --resume              # 从最新一帧快照接着跑
python -m CharAgent.client --help                # 全部选项 + 三步演示脚本
```
