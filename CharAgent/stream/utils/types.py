"""stream 包静态零件: 事件类型与事件对象 (issue 05 / #4).
...
class EventType(StrEnum):
    """事件类型全集 (与 03-api.md §2 的 SSE 事件名一一对应; SSE 的 event 字段).

    P1-7 的 HITL 挂起会再追加 approval_required —— 该事件由 loop 之外的
...
class StreamEvent:
    """一次运行产出的单个流式事件: 类型 + 序号 + 业务载荷 (issue 05 / #4).

    attributes:
        type: 事件类型 (EventType), 即 SSE 的 event 字段.
        seq: 事件序号, 每 run 从 1 起单调递增 —— 前端断点续拉的 after_event_id
            (03-api.md §2); P1 server 亦可将它映射为 SSE 的 id 字段.
        data: 业务载荷 (各类型字段清单见 03-api.md §2). 不含 run_id —— 框架层
            没有 run 概念, run_id 由 P1 server 在转发时注入.
    """
