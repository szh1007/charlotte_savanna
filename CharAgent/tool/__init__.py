"""工具层 (tool 包): @tool 装饰器 + JSON schema 生成 + 执行包装 (issue 03).

设计依据 (CharAgent/docs):
- #2: 工具错误必须「可操作」—— 说清字段期望/实际, 供 P0-3 loop 错误自纠错
...
- issue 03 用户决策: schema 生成 pydantic 为主 + manual 手写 typing 教学对照;
...
- tools_demo.py     演示工具集: 5 pydantic SOTA + 1 manual 对照 (业务无关,
                    供 P0-3 loop / P0-10 CLI 复用; 业务工具属 P1-13 demo 层)
