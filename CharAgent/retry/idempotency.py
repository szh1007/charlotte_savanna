...
(`retry/chat_model.py`), 工具执行是「跑一次 → 失败文本回填 → **模型自己
决定要不要再发一次**」(`tool/executor.py` 的 `execute_tool` 永不抛异常、
无循环、无超时), 而模型重发**是一次新的调用**, 不是同一个动作的重放.

2026-09-22 曾在这里记过「会产生重放的两条路径都还没做, 而且都跨进程, 所以
**两者是一对: 要么一起做, 要么都不做**」. **2026-09-24 改判**: 那条理由说的是
工具超时**依赖**本模块, 没说本模块依赖超时 —— 依赖方向是单向的, 可以拆.
于是只有一条路径现在做, 它也是本机制的第一个真实调用方:

1. **HITL 挂起-恢复 (做)**: 挂起半路 → 恢复 → 补做欠着的工具调用
   (`checkpoint/utils/pending.py`); 恢复被触发两次就会重放同一个
   `(run_id, message_id, tool_call_id)`. 这不是理论风险: 恢复是 HTTP 端点,
   双击 / 重发 / 重试都会产生第二次, 而重放的是「给这一单付款」.
   键是**三列**不是两列 —— 上游每轮从 `call_0` 重新编号, 少了 `message_id`
   在多轮之间会撞 (见 `db/schema.py`).
2. **工具超时 (推后, 仍然 P1)**: 超时会造成「结果未知」, 而「结果未知」必须先
   靠幂等或先查状态解决. 它现在**没有排期**, 理由与「#17 为什么零调用方」同一
