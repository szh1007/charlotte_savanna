...
**为什么至今零调用方 (2026-09-22 核实)**: 本模块挡的是「**同一个动作被执行
两次**」, 而今天框架里**不存在这条路径** —— 重试只包模型调用
(`retry/chat_model.py`), 工具执行是「跑一次 → 失败文本回填 → **模型自己
决定要不要再发一次**」(`tool/executor.py` 的 `execute_tool` 永不抛异常、
无循环、无超时), 而模型重发**是一次新的调用**, 不是同一个动作的重放.

会产生重放的两条路径都还没做, 而且**都跨进程** (P0 这个进程内 dict 记不住):

1. **HITL 挂起-恢复**: 挂起半路 → 恢复 → 补做欠着的工具调用
   (`checkpoint/utils/pending.py`); 恢复被触发两次就会重放同一个
   `(run_id, tool_call_id)` —— 这是本机制的第一个真实调用方.
2. **工具超时** (难点册 02-stability 的 P1): 超时会造成「结果未知」, 而
   「结果未知」必须先靠幂等或先查状态解决. **两者是一对**: 要么一起做,
   要么都不做.

届时要**连持久化存储一起补** (`db/README.md` 已规划 `idempotency_keys`
表, P1): P0 这个进程内 dict 记不住跨进程的重放, 而上面两条路径都跨进程.
