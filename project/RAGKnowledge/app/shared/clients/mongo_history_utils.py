...
class HistoryMongoTool:
    ...
    核心功能: 封装 MongoDB 的连接, 集合初始化, 索引创建, 为上层提供统一的数据库操作入口
    扩展功能: 支持与 LangChain 消息对象的格式转换(原代码预留能力)
    """

    def __init__(self):
        """
        类初始化方法: 完成 MongoDB 的连接, 数据库/集合获取, 索引创建
        ...
        try:
            ...
            # 获取对话记录的集合(相当于关系型数据库的表)
            # 集合名: chat_message  db.chat_message
            self.chat_message = self.db["chat_message"]

            # 为 chat_message 集合创建复合索引, 提升查询性能
            # 索引规则: session_id 升序 + ts 降序, 适配"按会话查最新记录"的核心查询场景
            # create_index 自带幂等性: 索引已存在时不会重复创建, 无需额外判断
...
# 定义全局变量: 存储 HistoryMongoTool 的单例实例
# 作用: 避免多次创建 HistoryMongoTool 实例, 从而避免重复建立 MongoDB 连接
_history_mongo_tool: HistoryMongoTool | None = None

# 模块加载时尝试初始化单例实例, 实现预加载
# 将数据库连接的初始化提前到模块加载阶段,
# 避免第一次调用接口时才建立连接(提升首次响应速度)
try:
    _history_mongo_tool = HistoryMongoTool()
except Exception as e:
    # 初始化失败时仅记录警告日志, 不抛出异常
    # 原因: 模块加载阶段的异常可能导致整个程序启动失败,
    # 此处保留懒加载兜底 (get_history_mongo_tool 会再次尝试创建)
...
def clear_history(session_id: str) -> int:
    ...
    """
    # 获取全局的 HistoryMongoTool 实例, 使用单例模式避免重复创建数据库连接
    mongo_tool = get_history_mongo_tool()
    try:
        # 执行批量删除操作: 删除所有 session_id 匹配的文档
        result = mongo_tool.chat_message.delete_many({"session_id": session_id})
        # 记录删除成功日志, 包含删除数量和会话 ID, 便于问题排查
        logger.info(f"Deleted {result.deleted_count} messages for session {session_id}")
        # 返回实际删除的数量(delete_many 的返回对象包含 deleted_count 属性)
        return result.deleted_count
    except Exception as e:
        # 捕获删除异常, 记录错误日志, 包含会话 ID
...
def save_chat_message(
    ...
    :param item_names: 关联的商品名称列表(可选, 支持多商品, 默认 None)
    :param image_urls: 关联的图片 URL 列表(可选, 默认 None)
    :param message_id: 记录主键 ID(可选, 有值则更新, 无值则新增)
    :return: 插入/更新的记录唯一标识
             新增返回 ObjectId 字符串, 更新返回传入的 message_id
    """
    # 生成当前时间的时间戳(秒级), 用于记录消息的创建时间, 后续用于排序和查询
    ts = datetime.now().timestamp()

    # 构造要插入/更新的文档数据(MongoDB 的基本数据单元是文档, 类似 Python 字典)
    document = {
        "session_id": session_id,  # 会话 ID, 关联维度
        "role": role,  # 消息角色
        "text": text,  # 消息内容
    ...
    # 获取全局的 HistoryMongoTool 实例, 使用单例模式
    mongo_tool = get_history_mongo_tool()
    # 判断是否传入主键 ID, 区分更新/新增逻辑
    if message_id:
        # 有 message_id: 执行更新操作(根据主键更新)
        result = mongo_tool.chat_message.update_one(
            {
                "_id": ObjectId(message_id)
            },  # 更新条件: 主键匹配(需将字符串转为 ObjectId 类型)
            {"$set": document},  # 更新操作: $set 表示只更新指定字段, 保留其他字段
        )
        # 更新操作返回传入的 message_id 作为标识
        return message_id
    else:
        # 无 message_id: 执行新增操作
        result = mongo_tool.chat_message.insert_one(document)
        # 新增操作返回插入的 ObjectId 并转为字符串,
        # 便于上层使用(避免直接返回 ObjectId 对象)
...
def update_message_item_names(ids: list[str], item_names: list[str]) -> int:
    ...
    mongo_tool = get_history_mongo_tool()
    try:
        # 将字符串类型的主键列表转为 MongoDB 的 ObjectId 类型
        # (数据库中主键是 ObjectId 类型)
        object_ids = [ObjectId(i) for i in ids]
        # 执行批量更新操作
        result = mongo_tool.chat_message.update_many(
            # 更新条件: 复合条件, 同时满足
            {"_id": {"$in": object_ids}},  # 主键在指定的 ID 列表中(批量筛选)
        ...
        # 返回实际更新的数量(modified_count: 真正被修改的文档数, 区别于 matched_count)
...
def get_recent_messages(session_id: str, limit: int = 10) -> list[dict[str, Any]]:
    ...
    mongo_tool = get_history_mongo_tool()
    try:
        # 构造查询条件: 仅查询指定 session_id 的记录
        query = {"session_id": session_id}

        # 执行查询: 按时间戳倒序取最近记录, 限制返回条数
        # find(query): 获取符合条件的游标(惰性加载, 不立即查询)
        # sort("ts", -1): 按 ts 字段倒序(从新到旧), 用于快速获取最近消息
