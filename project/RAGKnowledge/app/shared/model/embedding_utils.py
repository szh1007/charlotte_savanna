...
def get_bge_m3_ef() -> BGEM3EmbeddingFunction:
    ...
    if _bge_m3_ef is not None:
        logger.debug("BGE-M3 模型单例已存在, 直接返回实例")
        return _bge_m3_ef

    # 从环境变量加载配置, 无配置则使用默认值
    # 本地有可以使用本地地址, 没有的话使用 "BAAI/bge-m3" 会自动下载
    # 如果云端部署也可以使用 url 地址
    model_name = (
    ...
    try:
        ...
        _bge_m3_ef = BGEM3EmbeddingFunction(
            ...
            device=device,
            use_fp16=use_fp16,
            normalize_embeddings=True,  # 模型原生对稠密 + 稀疏向量做 L2 归一化
        )
        logger.success("BGE-M3 模型初始化成功, 已开启原生 L2 归一化")
        return _bge_m3_ef
    except Exception as e:
        logger.error(f"BGE-M3 模型初始化失败: {e!s}", exc_info=True)
        raise  # 向上抛出异常, 由调用方处理
...
def generate_embeddings(texts: list[str]) -> dict[str, list]:
    ...
    :return: 字典格式的向量结果, key 为 dense/sparse, 对应嵌套列表/字典列表
    ...
    if any(not isinstance(text, str) for text in texts):
        logger.warning("生成向量入参不合法, texts 中存在非字符串内容")
    ...
    try:
        ...
        # 构造最终返回结果, 稠密向量转列表(解决 numpy 数组不可序列化问题)
        result = {
            # embeddings["dense"] = [[1 稠密向量],[2 稠密向量],[...]  -> 1024]
            # embeddings["sparse"] = [[1 稀疏向量],[2 稠密向量],[...]  -> 1024]
            "dense": [
                emb.tolist() for emb in embeddings["dense"]
            ],  # 嵌套列表, 与输入文本一一对应
            "sparse": processed_sparse,  # 字典列表, 模型已做 L2 归一化
        }
        logger.success(f"{len(texts)}条文本向量生成完成, 格式已适配工业级使用")
        return result

    except Exception as e:
        logger.error(f"文本向量生成失败: {e!s}", exc_info=True)
