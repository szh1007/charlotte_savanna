...
def get_path_dir(ps: int = 0) -> Path:
    """
    pathlib.Path 提供了 parents 属性, 这是一个有序的路径上级目录迭代器,
    直接通过索引取值就能快速获取[上 N 级目录], 完美解决多层 .parent 繁琐的问题,
    这也是官方推荐的简化写法
    ...
    Args:
        ps: 索引值, 表示[上 N 级目录](默认 0, 即当前文件所在目录)
    Returns:
        Path: 目录路径对象, 表示[上 N 级目录]
    """
