...
#       当 User A 的请求正在处理时, User B 的请求进来了. 如果修改了全局变量,
#       User A 的数据就会被 User B 覆盖, 导致严重的 "串台" 事故
#       (例如 User A 的文件存到了 User B 的目录).
#
#    2. 如果使用 threading.local:
#       它是基于线程隔离的. 因为 asyncio 所有协程都在同一个线程跑,
#       所以 threading.local 在异步场景下失效, 无法隔离不同用户的请求.
#
#    3. ContextVar 的解决方案:
#       ContextVar 是 Python 3.7+ 专门为异步编程设计的 "协程级局部变量".
#       它能确保变量在每一个 asyncio Task (即每个用户请求) 中是 *独立隔离* 的.
#       无论代码调用多深, 只要是在同一个请求链路(Context)中,
#       get() 到的都是属于当前请求的数据.
# ============================================================================
...
# - 作用: 用来记录 "当前是谁在执行任务".
# - 场景: 当 Agent 打印日志或者通过 WebSocket 给前端发消息时,
#   它需要知道: "我现在是正在服务张三, 还是李四?" 这样消息才不会发错人.
...
if __name__ == "__main__":
    ...
    async def deep_nested_function(user_name):
        ...
        if user_name in current_thread and user_name in current_dir:
            print(f"[{user_name}] 3. ok 验证成功! 数据是正确的!")
        else:
            print(f"[{user_name}] 3. ng 验证失败! 数据串台了!")
