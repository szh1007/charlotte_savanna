...
async def start_kb_index(req: KbIndexRequest):
    """知识库索引任务 (POST /ai/kb/index): 创建异步任务.

    真实索引: 解析 → 切分 → embedding → Milvus 入库, SSE
...
async def task_events(task_id: str, request: Request):
    """SSE 进度流: event 名 pipeline-progress, 帧带递增 id.

    断线重连客户端自动携带 Last-Event-ID, 服务端从增量续推.
    """
    task = await task_system.get_task(task_id)
    if task is None:
        raise HTTPException(status_code=404, detail="任务不存在或已过期")
